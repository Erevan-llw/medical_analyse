...
def selectDepartmentId(s_list, string):
    """找到str所对应的数据"""
    return s_list.index(string) + 1
...
if __name__ == '__main__':
    ...
    for paper in papers:
        ill_dic = {'name': setName(paper),
                   'get_prob': setGetProb(paper),
                   'get_prob_1000': str(setGetProb1000(setGetProb(paper))),
                   'get_prob_note': setGetProbNote(paper),
                   'desc': setDesc(paper),
                   'acompany': setAcompany(paper),
                   'cure_lasttime': setCure_lasttime(paper),
                   'symptom': setSymptom(paper),
                   'cause': setCause(paper),
                   'cure_department': setCure_department(paper),
                   'department_id': selectDepartmentId(c_name, setCure_department(paper)),
                   'cure_way': setCure_way(paper),
                   'do_eat': setDo_eat(paper),
                   'not_eat': setNot_eat(paper),
                   'yibao_status': setYibao_status(paper),
                   'easy_get': setEasy_get(paper),
                   'get_way': setGet_way(paper),
                   'check': setCheck(paper),
                   'drug_detail': setDrug_detail(paper),
                   'recommand_drug': setRecommand_drug(paper),
                   'cured_prob': str(setCuredProb(paper)),
                   'cured_prob_note': setCuredProbNote(paper)}
        cleaned_papers.append(ill_dic)
    count = 1
    for paper in cleaned_papers:
        insert_query = "INSERT INTO `illness` (`clicked_count`,`subscribe_count`,`name`,`get_prob`,`get_prob_1000`," \
                       "`get_prob_note`,`desc`," \
                       "`acompany`,`cure_lasttime`,`symptom`, `cause`, `cure_department`,`department_id`,`cure_way`," \
                       "`do_eat`,`not_eat`,`yibao_status`,`easy_get`,`get_way`,`check`,`drug_detail`,`recommand_drug`,"\
                       "`cured_prob`,`cured_prob_note` ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s," \
                       "%s,%s,%s,%s,%s,%s,%s) "
        print(cursor.execute(insert_query,
                             (0, 0, paper['name'], paper['get_prob'], paper['get_prob_1000'], paper['get_prob_note'],
                              paper['desc'], paper['acompany'], paper['cure_lasttime'], paper['symptom'],
                              paper['cause'], paper['cure_department'],paper['department_id'], paper['cure_way'], paper['do_eat'],
